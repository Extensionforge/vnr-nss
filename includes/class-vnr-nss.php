...
add_action('set_logged_in_cookie', 'custom_get_logged_in_cookie_nss', 10, 6);
function custom_get_logged_in_cookie_nss($logged_in_cookie, $expire, $expiration, $user_id, $logged_in_text, $token)
{
	callApi($user_id);
}
...
class Vnr_Nss {
	 ...
	 * @since    1.0.0
	 * @access   protected
	 * @var      Vnr_Nss_Loader    $loader    Maintains and registers all hooks for the plugin.
	 */
	protected $loader;
	 ...
	 */
	public function __construct() {
		if ( defined( 'VNR_NSS_VERSION' ) ) {
			$this->version = VNR_NSS_VERSION;
		} else {
			$this->version = '1.0.0';
		}
		$this->plugin_name = 'vnr-nss';
	 ...
	 *
	 * - Vnr_Nss_Loader. Orchestrates the hooks of the plugin.
	 * - Vnr_Nss_i18n. Defines internationalization functionality.
	 * - Vnr_Nss_Admin. Defines all hooks for the admin area.
	 * - Vnr_Nss_Public. Defines all hooks for the public side of the site.
	 *
	...
	private function load_dependencies() {
		 ...
		 * of the plugin.
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/class-vnr-nss-i18n.php';

		/**
		 * The class responsible for defining all actions that occur in the admin area.
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'admin/class-vnr-nss-admin.php';

		/**
		 * The class responsible for defining all actions that occur in the public-facing
		 * side of the site.
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'public/class-vnr-nss-public.php';

		$this->loader = new Vnr_Nss_Loader();
	 ...
	 *
	 * Uses the Vnr_Nss_i18n class in order to set the domain and to register the hook
	 * with WordPress.
	 *
	 * @since    1.0.0
	 * @access   private
	...
	private function define_admin_hooks() {
		...
		$plugin_admin = new Vnr_Nss_Admin( $this->get_plugin_name(), $this->get_version() );
	...
	private function define_public_hooks() {
		...
		$plugin_public = new Vnr_Nss_Public( $this->get_plugin_name(), $this->get_version() );
	 ...
	 *
	 * @since     1.0.0
	 * @return    Vnr_Nss_Loader    Orchestrates the hooks of the plugin.
	 */
